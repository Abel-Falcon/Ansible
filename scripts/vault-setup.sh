...
show_help() {
    ...
    echo "  gen-passwords - Generar contraseñas para usuarios"
...
# Funciones para encriptar, desencriptar, editar y ver vault
encrypt_vault() {
...
test_vault() {
    ...
    # Probar acceso a variables para Ubuntu y Windows
    echo -e "${GREEN}🔍 Probando acceso a variables y conectividad...${NC}"
    ansible ubuntu -i inventories/hosts -m ping -e "@$VAULT_FILE" &>/dev/null
    ansible windows -i inventories/hosts -m win_ping -e "@$VAULT_FILE" &>/dev/null
